...
static int	is_previous(t_room *pass, int indx, int compare)
{
	int	i;

	i = 0;
	while (pass->prev_indx[indx][i] != -1)
	{
		if (pass->prev_indx[indx][i] == compare)
			return (TRUE);
		++i;
	}
	i = 0;
	while (pass->prev_path[indx][i] != -1)
	{
		if (pass->prev_path[compare][i] == indx)
			return (TRUE);
		++i;
	}
	return (FALSE);
